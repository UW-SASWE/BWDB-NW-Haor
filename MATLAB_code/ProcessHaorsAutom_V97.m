...
for dd=1:1
    ...
    inpath = 'D:\SASWMS_Shahzaib\barind\ProcessHaorsAutom_V97\for_redistribution_files_only\sardata\Barind_SAR_Haors_Smooth_';
    infile = [inpath dtf  '.tif'];
    display(infile);
    ...
    if isfile(infile)
        ...
        %CC.NumObjects;
        latlim = [23.891862791028032 26.3748960678628];
        lonlim = [88.12347173072729 89.6891454394192];
        cellExtent = 0.08;
        R = georefcells(latlim,lonlim,  ...
        cellExtent,cellExtent,'ColumnsStartFrom','north');
        R.RasterSize = size(binaryImage3);
        outfile =  ['D:\SASWMS_Shahzaib\barind\ProcessHaorsAutom_V97\for_redistribution_files_only\outdata\Processed_Haor_Barind_' dt  '.tif'];
        display('outfile', outfile);
        geotiffwrite(outfile,binaryImage3,R);
        ...
        xt=x';
        vol=0.0021*([stats.Area].*8e-5);  %*0.0056 - 0.03; 
